...
export const TableDoneOrders = () => {
  ...
  const lastDoc = useAdminStore(state => state.lastDoc);
  ...
  const handleChangePage = (event: unknown, newPage: number) => {
    void fetchDoneOrders(newPage > page ? 'next' : 'prev');
    setPage(newPage);
  };
        ...
        <Table>
          ...
          <TableBody>
            {isLoading && orders.length === 0 ? (
              ...
              paginatedOrders.map(order => <OrderDoneTableRow key={order.id} order={order} />)
            )}
          </TableBody>
        </Table>
