...
import { Restaurant, Kitchen, CheckCircle, LocalShipping } from "@mui/icons-material";
...
export const OrderStatsChips = ({ stats }: OrderStatsChipsProps) => {
    return (
        ...
        >
            ...
            <Chip 
                icon={<Kitchen />} 
                label={`Готовятся: ${stats.cooking}`} 
                color="warning" 
                variant="outlined" 
            />
            <Chip 
                icon={<CheckCircle />} 
                label={`Готовы: ${stats.ready}`} 
                color="info" 
                variant="outlined" 
            />
            <Chip 
                icon={<LocalShipping />} 
                label={`В доставке: ${stats.in_delivery}`} 
                color="primary" 
                variant="outlined" 
            />
        </Stack>
