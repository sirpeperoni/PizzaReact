import { useRef } from 'react';
import { useFrame } from '@react-three/fiber';
import * as THREE from 'three';
import type { InstanceData } from './positioning';

interface PepperoniProps {
  instances: InstanceData[];
  yBase: number;
}

export const Pepperoni = ({ instances, yBase }: PepperoniProps) => {
  const groupRef = useRef<THREE.Group>(null!);
  const yRef = useRef(2.0);

  useFrame((_state, delta) => {
    yRef.current = THREE.MathUtils.lerp(yRef.current, 0, 1 - Math.exp(-8 * delta));
    if (groupRef.current) groupRef.current.position.y = yBase + yRef.current;
  });

  return (
    <group ref={groupRef} position={[0, yBase, 0]}>
      {instances.map((inst, i) => (
        <mesh key={i} position={[inst.x, 0, inst.z]} rotation={[0, inst.rotation, 0]} scale={inst.scale} castShadow>
          <cylinderGeometry args={[0.18, 0.18, 0.025, 16]} />
          <meshStandardMaterial color='#c0392b' roughness={0.6} />
        </mesh>
      ))}
    </group>
  );
};
