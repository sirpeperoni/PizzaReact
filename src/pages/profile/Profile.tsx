  ...
  return (
    <Container maxWidth='xl' sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {inProgressOrders.length > 0 && <ProgressOrders inProgressOrders={inProgressOrders} />}

        {completedOrders.length > 0 && <CompletedOrders completedOrders={completedOrders} />}

        {history.length === 0 && !isLoading && <NoOrders />}
      </Box>
    </Container>
  );
};
